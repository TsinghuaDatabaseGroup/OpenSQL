...
if [[ $# -lt 3 ]]; then
    echo "Usage: bash launch_global_sft.sh <lr> <epoch> <model_name_or_path> [finetune_data_path] [storage_dir] [seed]" >&2
    exit 1
fi
...
default_data="${PROJECT_ROOT}/artifacts/training_data/global_schema_linking_sft.json"
finetune_data_name="${4:-${FINETUNE_DATA_PATH:-${default_data}}}"
default_storage="${PROJECT_ROOT}/artifacts/trained_models/global_linker_sft"
